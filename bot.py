...
# ---------- CONFIG ----------
# Token embedded (as requested) – but use .env in production for security!
BOT_TOKEN = "8639978539:AAE3rkrUMELtU74Gn7XidLbz_z1lIp_vBX8"
OWNER_ID = 8745088070
DB_PATH = "bot_data.db"
...
def is_bot_admin(chat_id, context):
    try:
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "sett_welcome":
        ...
        import json
        row = db_query("SELECT settings FROM groups WHERE chat_id = ?", (chat_id,), fetch_one=True)
        sett = json.loads(row['settings']) if row else {}
        current = sett.get('antispam', True)
        sett['antispam'] = not current
        db_query("UPDATE groups SET settings = ? WHERE chat_id = ?", (json.dumps(sett), chat_id))
        await query.edit_message_text(f"🛡️ Anti-Spam {'enabled' if sett['antispam'] else 'disabled'}.")
    elif data == "sett_links":
        import json
        row = db_query("SELECT settings FROM groups WHERE chat_id = ?", (chat_id,), fetch_one=True)
        sett = json.loads(row['settings']) if row else {}
        current = sett.get('linkfilter', True)
        sett['linkfilter'] = not current
        db_query("UPDATE groups SET settings = ? WHERE chat_id = ?", (json.dumps(sett), chat_id))
        await query.edit_message_text(f"🔗 Link filter {'enabled' if sett['linkfilter'] else 'disabled'}.")
    elif data == "sett_badwords":
        import json
        row = db_query("SELECT settings FROM groups WHERE chat_id = ?", (chat_id,), fetch_one=True)
        sett = json.loads(row['settings']) if row else {}
        current = sett.get('badwords', True)
        sett['badwords'] = not current
        db_query("UPDATE groups SET settings = ? WHERE chat_id = ?", (json.dumps(sett), chat_id))
        await query.edit_message_text(f"🚫 Bad-word filter {'enabled' if sett['badwords'] else 'disabled'}.")
...
async def group_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    import json
    sett = json.loads(row['settings']) if row else {}
    antispam = sett.get('antispam', True)
    linkfilter = sett.get('linkfilter', True)
    badwords = sett.get('badwords', True)
    ...
    # Repeated message
    if hasattr(context.bot_data, 'last_msgs'):
        last = context.bot_data.get('last_msgs', {}).get(key)
        if last and last == update.message.text:
            try:
                ...
                pass
        context.bot_data['last_msgs'] = context.bot_data.get('last_msgs', {})
...
async def captcha_timeout(chat_id, user_id, timeout, msg_id):
    ...
    # Check if already verified (we store verified in group_members)
...
# ---------- OWNER PANEL ----------
BROADCAST_STATE = 1 count FROM groups", fetch_one=True)
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        ...
        return
    groups = db_query("SELECT COUNT(*) as    users = db_query("SELECT COUNT(DISTINCT user_id) as count FROM group_members", fetch_one=True)
    keyboard = [
    ...
    if data == "broadcast_confirm":
        ...
        for g in groups:
            try:
                ...
                sent += 1
                await asyncio.sleep(0.07)  # rate limit
            except Exception as e:
                failed += 1
...
def main():
    ...
    app.add_handler(CommandHandler("broadcast", broadcast_message_handler))  # will be intercepted
